...
class TriggerWatcher:
    ...
    def _row_time(self, row: dict[str, Any]) -> Optional[float]:
        for key in ("timestamp", "time", "datetime", "date_time", "created_at"):
            value = row.get(key)
            if value in (None, ""):
                continue
            if isinstance(value, (int, float)):
    ...
    def _latest_price(self, rows: list[dict[str, Any]]) -> tuple[Optional[float], str, str]:
        ...
        for key in ("price", "close", "last", "ltp", "option_price", "value"):
    ...
    def _row_signature(self, row: dict[str, Any] | None) -> str:
        if not row:
            return ""
        important = []
        for key in ("timestamp", "time", "datetime", "date_time", "price", "close", "last", "ltp", "option_price", "volume", "signal", "comment", "source_tab", "_source_tab"):
            if key in row:
    ...
    def _watch_reason(self, data_status: str, call_price: Optional[float], put_price: Optional[float], call_source: str, put_source: str) -> str:
        return (
            "Watching live sheet rows; no LLM trigger zone touched yet | "
            f"data={data_status} | call={call_price} from {call_source or 'CALLS'} | "
            f"put={put_price} from {put_source or 'PUTS'}"
        )
    ...
    def check(self, call_rows: list[dict[str, Any]], put_rows: list[dict[str, Any]]) -> WatchResult:
        ...
        if self._in_zone(call_price, self.plan.get("call_battle_area", {})):
            return WatchResult(action="START_BATTLE", trigger_type="CALL_BATTLE_ZONE_TRIGGER", reason=f"LIVE SHEET DATA touched LLM CALL battle zone | call={call_price} from {call_source or 'CALLS'}", data_status=data_status, latest_call_price=call_price, latest_put_price=put_price, call_source=call_source, put_source=put_source)

        if self._in_zone(put_price, self.plan.get("put_battle_area", {})):
            return WatchResult(action="START_BATTLE", trigger_type="PUT_BATTLE_ZONE_TRIGGER", reason=f"LIVE SHEET DATA touched LLM PUT battle zone | put={put_price} from {put_source or 'PUTS'}", data_status=data_status, latest_call_price=call_price, latest_put_price=put_price, call_source=call_source, put_source=put_source)
        ...
        if self._in_zone(call_price, consolidation) or self._in_zone(put_price, consolidation):
            return WatchResult(action="START_BATTLE", trigger_type="CONSOLIDATION_ZONE_TRIGGER", reason=f"LIVE SHEET DATA touched LLM consolidation zone | call={call_price} | put={put_price}", data_status=data_status, latest_call_price=call_price, latest_put_price=put_price, call_source=call_source, put_source=put_source)

        if next_action == "START_BATTLE":
            return WatchResult(action="START_BATTLE", trigger_type="LLM_DIRECT_TRIGGER", reason="LLM next_action requested immediate battle session", data_status=data_status, latest_call_price=call_price, latest_put_price=put_price, call_source=call_source, put_source=put_source)
